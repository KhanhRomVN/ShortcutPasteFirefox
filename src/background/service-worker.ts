...
(function () {
    ...
    async getClipboardItems(): Promise<any[]> {
      try {
        ...
        if (browserAPI.storage.sync) {
          const syncResult = await browserAPI.storage.sync.get(
          ...
          if (syncResult[this.STORAGE_KEYS.ITEMS]) {
            ...
            await storage.set({
              [this.STORAGE_KEYS.ITEMS]: syncResult[this.STORAGE_KEYS.ITEMS],
            });
          ...
          if (itemsSize <= 80000) {
            try {
              await browserAPI.storage.sync.set({
                [this.STORAGE_KEYS.ITEMS]: items,
              });
              logger.debug("Also synced to sync storage");
            } catch (syncError) {
              logger.warn(
                "Sync storage failed, but local storage succeeded:",
                syncError
              );
            }
          } else {
  ...
  class ContentScriptManager {
    ...
    static async sendMessageToTab(
      ...
      for (let i = 0; i < retries; i++) {
        try {
          logger.debug(
            `Sending message to tab ${tabId} (attempt ${i + 1}/${retries})`,
            {
          ...
          const finalMessage = {
            ...message,
            action:
              message.action === "pasteClipboardItem"
                ? "pasteDirectValue"
                : message.action,
          };
          ...
          logger.error(
            `Failed to send message to tab ${tabId} (attempt ${i + 1}):`,
            {
              error: error instanceof Error ? error.message : String(error),
              tabId,
            }
    ...
    static async isContentScriptInjected(tabId: number): Promise<boolean> {
      try {
        ...
        logger.debug(
          `Content script injection check for tab ${tabId}: ${
            isInjected ? "INJECTED" : "NOT_INJECTED"
          }`
        );
        return isInjected;
      } catch (error) {
        logger.debug(
          `Content script not detected in tab ${tabId}:`,
    ...
    static isTabSupported(tab: any): boolean {
      ...
      const isSupported = !unsupportedUrls.some((prefix) =>
  ...
  logger.info("Background Script initialized", {
    browser:
      typeof (globalThis as any).browser !== "undefined" ? "Firefox" : "Chrome",
    ...
    browserAPI.commands
      ...
      .then((commands: any[]) => {
        ...
        if (commands.length === 0) {
          logger.warn(
            "⚠️ No commands found! Extension may need to be reinstalled."
          );
        }
      })
      .catch((error: any) => {
  ...
  async function handlePasteFavoriteCommand(tab?: any) {
    ...
    try {
      ...
      // Find favorite item
      const favoriteItem = clipboardItems.find(
        (item) => item && typeof item === "object" && item.isFavorite === true
      );
      ...
      // Attempt paste
      logger.info(
        `📝 Attempting to paste content (${favoriteItem.content.length} chars) to tab ${activeTab.id}`
      );

      const response = await ContentScriptManager.sendMessageToTab(
        activeTab.id,
        {
      ...
      const duration = Date.now() - startTime;
      logger.error(
        `💥 Critical error in handlePasteFavoriteCommand (${duration}ms):`,
        {
          error: error instanceof Error ? error.message : String(error),
          stack: error instanceof Error ? error.stack : undefined,
        }
